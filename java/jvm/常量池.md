...
在class文件中可以查看类中的常量。class常量池是java类文件的一部分，由编译器在编译java源文件时产生，存储在.class文件中，包括类或者接口的字面量 和符号引用。
...
在运行过程中，存储的的 常量、方法、字段的引用，以及字符串字面量  在元空间
...
用于存储字符串字面量的内存区域， 在java7之后 字符串常量池移到堆内存中。更好的垃圾回收