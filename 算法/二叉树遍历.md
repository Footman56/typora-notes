...
```java
 /**
     ...
     * @param root
     */
    public static  void preOrder1(BTreeNode root) {
        if (null == root) {
            return;
        }
...
/**
     ...
     * @param root
     */
    public static void inOrder1(BTreeNode root) {
        // 需要先找到最左边的节点
        if (null == root) {
            return;
        }
        ...
        do {
            ...
            if (root != null) {
                ...
                // 当前root的左节点为null了，说明没有最左边的节点了，就可以处理当前节点了
                root = stack.pop();
                System.out.println(root.getValue());
                // 处理右子树
                root = root.getRight();
            }
            // 但是有可能队列中没有数据啦，但是还有右节点此时还是需要处理的
...
1. 可以先用一层循环来模拟找最左边的节点
2. 检查栈顶节点是否可以打印（检查原理是 这个节点是不是作为小树的根）
3. 如果能打印就处理；不能打印还需入栈
4. 设置节点开启下一次的循环

```java
    public static void postOrder1(BTreeNode root) {
        if (null == root) {
            return;
        }
        Stack<BTreeNode> stack = new Stack<>();
        ...
        do {
            ...
            // 获取最左边的节点
            BTreeNode node = stack.pop();
            // 检查最左边的节点是否能打印: 没有右边的节点或者右边的节点已经打印过了
            if (node.getRight() == null || node.getRight() == preNode) {
                ...
                // 此时不能打印这个节点，因为还有右边的节点没有处理。那么就需要重新放入堆栈中
                stack.push(node);
                // 开始处理右边的节点
                root = node.getRight();
            }
