...
### Q、K、V 的形状：(batch, heads, L_q, d_k)?

+ **一次训练多个句子 → batch 维度**

+ **每个句子会被分成多个注意力头 → heads 维度**

+ **每个句子有多个 token → L_q 维度 (sequence length)**

+ **每个 token 是一个 d_k 维的向量 → d_k 维度**

需要多个头来学习不同的关系

```
# 形状形成过程
1. 原始 embedding：  X : (batch, L_q, d_model)
2. 乘以 W_Q（维度 = d_model × d_model）：  Q_raw = X @ W_Q     → (batch, L_q, d_model)
3. reshape： Q_raw.reshape(batch, L_q, heads, d_k)
4. 再transpose： Q = Q_raw.transpose(1, 2)
5. 最终形状为：Q = (batch, heads, L_q, d_k)
```
...
3. embedding 通过三组线性层生成三个子空间：
   ...
   4. Q = E @ W_Q; K = E @ W_K ;V = E @ W_V 
   W_Q 是(64, 64)  随机矩阵，后面会通过学习不断调整
   ```
...
<img src="/Users/peilizhi/Library/Application Support/typora-user-images/image-20251130141435829.png" alt="image-20251130141435829" style="zoom:50%;" />

注意力分数本质是查询 **查询 Q 与所有键 K 的相似度**

点积是计算相似度的一种方法

+ 点积大 → 两个向量方向接近 → 这两个词强相关
